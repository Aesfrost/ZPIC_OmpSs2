		...
		float box[], float dt, t_current *global_current, t_region *prev_region)
{
	...
	free(region->local_current.J_buf);
	region->local_current.J = global_current->J + region->limits_y[0] * region->local_current.nrow;
	region->local_current.J_buf = region->local_current.J - region->local_current.gc[0][0]
			- region->local_current.gc[1][0] * region->local_current.nrow;
	...
	if (id + 1 < n_regions)
	{
		...
		region_new(region->next, n_regions, nx, id + 1, n_spec, spec, box, dt, global_current, region);
	} else
	{
...
void region_reset_current(t_region *region)
{
	const int start = region->id == 0 ? 0 : region->local_current.gc[1][0] * region->local_current.nrow;
	int size;

	if(region->id == 0) size = region->nx[1] + region->local_current.gc[1][0];
	else if(region->next->id == 0) size = region->nx[1] + region->local_current.gc[1][1];
	else size = region->nx[1];
	size *= region->local_current.nrow;

	current_zero(&region->local_current, start, size);
	if (region->next->id != 0) region_reset_current(region->next);
}
...
{
	const int begin = region->id == 0 ? -region->local_current.gc[1][0] : 0;
	int end = region->next->id == 0 ? region->nx[1] + region->local_current.gc[1][1] : region->nx[1];

	current_reduction_x(&region->local_current, begin, end);
...
void region_current_smooth(t_region *region, enum CURRENT_SMOOTH_MODE mode)
{
	switch (mode)
	{
		...
		case CURRENT_UPDATE_GC:
			if(region->id != 0) return; //ONLY for the first region!
