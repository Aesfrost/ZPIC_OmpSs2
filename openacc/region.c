...
void region_init(t_region *region)
{
	...
	current_overlap_zone(&region->local_current, &region->prev->local_current, region->id % num_gpus);
	emf_overlap_zone(&region->local_emf, &region->prev->local_emf, region->id % num_gpus);

#ifdef ENABLE_PREFETCH
	grid_prefetch_openacc(region->local_current.J_buf, region->local_current.total_size, region->id % num_gpus, NULL);
	grid_prefetch_openacc(region->local_emf.E_buf, region->local_emf.total_size, region->id % num_gpus, NULL);
	grid_prefetch_openacc(region->local_emf.B_buf, region->local_emf.total_size, region->id % num_gpus, NULL);
#endif
...
#ifdef ENABLE_PREFETCH
		spec_prefetch_openacc(&region->species[i].main_vector, region->id % num_gpus, NULL);
#endif
	}
}
