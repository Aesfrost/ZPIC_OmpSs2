...
// CPU Tasks
//#pragma omp task out(current->J_buf[0; current->total_size]) label(Current Reset)
void current_zero(t_current *current);

//#pragma omp task depend(inout : current->J_buf[0: current->overlap_zone]) \
//depend(inout :current->J_upper[-current->gc[0][0]: current->overlap_zone]) \
//label(Current Reduction Y)
void current_reduction_y(t_current *current); // Each region only update the zone in the top edge

//#pragma omp task depend(inout :current->J_buf[0: current->total_size]) label(Current Reduction X)
...
//#pragma omp task depend(inout :current->J_buf[0: current->total_size]) label(Current Smooth X)
void current_smooth_x(t_current *current);

//#pragma omp task depend(inout : current->J_buf[0: current->total_size]) label(Current Smooth Y)
