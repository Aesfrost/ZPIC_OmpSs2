 ...
 */
enum zdf_data_type {
	zdf_null, zdf_int8, zdf_uint8, zdf_int16, zdf_uint16, zdf_int32, zdf_uint32, zdf_int64,
	zdf_uint64, zdf_float32, zdf_float64
};
	...
	switch (mode)
	{
		...
		case ZDF_READ:
			...
			for (int i = 0; i < ZDF_MAGIC_LENGTH; i++)
			{
				...
				{
					fprintf(stderr,
							"(*error*) Invalid magic number, file is not a proper ZDF file.");
					zdf_close_file(zdf);
					return (-1);
				}
			}
...
int zdf_add_string(t_zdf_file *zdf, char *name, const char *str)
{
	...
	t_zdf_record rec = {.id_version = ZDF_STRING_ID, .name = name, .length = size_zdf_string(str)};
...
int zdf_add_int32(t_zdf_file *zdf, char *name, const int32_t value)
{
	...
	t_zdf_record rec = {.id_version = ZDF_INT32_ID, .name = name, .length = size_zdf_int32};
...
{
	t_zdf_record rec = {.id_version = ZDF_DOUBLE_ID, .name = name, .length = size_zdf_double};
...
int zdf_add_iteration(t_zdf_file *zdf, const char *name, const t_zdf_iteration *iter)
{
	...
	t_zdf_record rec = {.id_version = ZDF_ITERATION_ID, .name = (char*) name,
						.length = size_zdf_uint32 + size_zdf_double
...
uint64_t size_xdr_zdf_grid_info(const t_zdf_grid_info *grid)
{
	...
	if (grid->axis)
	{
		...
		for (unsigned int i = 0; i < grid->ndims; i++)
			size += size_zdf_int32 + 2 * size_zdf_double + size_zdf_string(grid->axis[i].label)
...
int zdf_add_grid_info(t_zdf_file *zdf, const char *name, const t_zdf_grid_info *grid)
{
	...
	t_zdf_record rec = {.id_version = ZDF_GRID_INFO_ID, .name = (char*) name,
...
uint64_t size_xdr_zdf_part_info(const t_zdf_part_info *part)
{
	...
	{
		size += size_zdf_string(part->quants[i]) + size_zdf_string(part->units[i]);
	}
...
int zdf_add_part_info(t_zdf_file *zdf, char *name, t_zdf_part_info *part)
{
	...
	t_zdf_record rec = {.id_version = ZDF_PART_INFO_ID, .name = name,
...
uint64_t size_zdf_dataset(const t_zdf_dataset *dataset)
{
	...
	return size_zdf_int32 + size_zdf_uint32 + dataset->ndims * size_zdf_uint64 + data_size;
}

int zdf_add_dataset(t_zdf_file *zdf, char *name, t_zdf_dataset *dataset)
{

	t_zdf_record rec = {.id_version = ZDF_DATASET_ID, .name = name, .length = size_zdf_dataset(
			dataset)};
		...
		const t_zdf_iteration *_iteration, char const path[])
{
	...
	// Set iteration info
	t_zdf_iteration iteration = {.n = _iteration->n, .t = _iteration->t,
	...
	if (_info->axis)
	{
		...
		for (i = 0; i < _info->ndims; i++)
		{
			axis[i] = (t_zdf_grid_axis)
					{.type = zdf_linear, .min = _info->axis[i].min, .max = _info->axis[i].max,
						.label = _info->axis[i].label, .units = _info->axis[i].units};
		}
	...
	} else
	{
		...
		{
			axis[i] = (t_zdf_grid_axis)
					{.type = zdf_linear, .min = 0, .max = _info->nx[i], .label = "", .units = ""};
		}
	}

	// Set grid info
	t_zdf_grid_info grid_info = {.ndims = _info->ndims, .label = _info->label,
									.units = _info->units, .axis = axis};
	for (i = 0; i < _info->ndims; i++)
		grid_info.nx[i] = _info->nx[i];

	// Set data
	t_zdf_dataset dataset = {.data_type = zdf_float32, .ndims = _info->ndims,
								.data = (uint8_t*) data};
	for (i = 0; i < _info->ndims; i++)
		dataset.nx[i] = _info->nx[i];
...
int zdf_part_file_open(t_zdf_file *zdf, t_zdf_part_info *_info, const t_zdf_iteration *_iteration,
		char const path[])
{
...
int zdf_part_file_add_quant(t_zdf_file *zdf, const char *name, const float *data, const unsigned np)
{

	t_zdf_dataset dataset = {.data_type = zdf_float32, .ndims = 1, .data = (uint8_t*) data};
