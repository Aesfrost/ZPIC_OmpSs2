...
typedef struct {
	...
	int total_size; // Total size of the EMF buffer
	int overlap_size;  // The number of cells in the overlap zone
...
// OpenAcc Tasks
#pragma oss task label("EMF Advance (GPU)") device(openacc) \
...
#pragma oss task label("EMF Update GC (GPU)") device(openacc) \
	inout(emf->B_buf[0; emf->overlap_size]) \
	inout(emf->B_below[-emf->gc[0][0]; emf->overlap_size]) \
	inout(emf->E_buf[0; emf->overlap_size]) \
	inout(emf->E_below[-emf->gc[0][0]; emf->overlap_size])
