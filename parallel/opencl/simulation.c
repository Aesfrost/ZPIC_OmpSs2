...
#ifdef TARGET_GPU
	sprintf(filename, "output/%s/iter_timings_gpu.csv", sim->name);
#elif TARGET_FPGA
	sprintf(filename, "output/%s/iter_timings_fpga.csv", sim->name);
#else
	sprintf(filename, "output/%s/iter_timings.csv", sim->name);
#endif
...
void sim_set_moving_window(t_simulation *sim)
{
	...
	for (int i = 0; i < sim->n_species; i++)
		spec_set_moving_window(&sim->species[i]);
}
...
void sim_timings(t_simulation *sim, uint64_t t0, uint64_t t1)
{
	...
	for (i = 0; i < sim->n_species; i++)
		npart += sim->species[i].part_vector.np;
...
void sim_iter_timings(t_simulation *sim, const int iter, const uint64_t t0, const uint64_t t1)
{
	char filename[128];
	FILE *file;
	...
	if (file)
	{
		fprintf(file, "%d,%f\n", iter, timer_interval_seconds(t0, t1));
		fclose(file);
