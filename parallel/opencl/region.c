...
void region_current_reduction_x(t_region *region)
{
	...
	if(region->enable_gpu) current_reduction_x(&region->local_current);
...
void region_current_smooth(t_region *region, enum CURRENT_SMOOTH_MODE mode)
{
	...
	{
		case SMOOTH_X:
			if(region->enable_gpu) current_smooth_x(&region->local_current);
...
void region_emf_advance(t_region *region, enum EMF_UPDATE mode)
{
	...
	{
		case EMF_ADVANCE:
			if(region->enable_gpu) emf_advance(&region->local_emf, &region->local_current);
