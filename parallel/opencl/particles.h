...
// CPU Tasks
#pragma omp task label(Spec Advance) \
...
#pragma omp task inout(spec->main_vector) out(lower_spec->temp_buffer[1]) \
...
#pragma omp task in(spec->temp_buffer[0:1]) inout(spec->main_vector) label(Spec Update)
void spec_update_main_vector(t_species *spec);

// OpenAcc Tasks
//#pragma omp task label(Spec Kernel (GPU)) device(openacc) \
//	in(emf->E_buf[0; emf->total_size]) in(emf->B_buf[0; emf->total_size]) \
...
//
//#pragma omp task label(Spec Post Processing (GPU)) device(openacc) \
...
//
//#pragma omp task label(Spec Update (GPU)) in(spec->temp_buffer[0:1]) inout(spec->main_vector)
//void spec_post_processing_2_openacc(t_species *restrict spec, const int limits_y[2]);
//
//#pragma omp task label(Spec Sort (GPU)) weakinout(spec->main_vector)
