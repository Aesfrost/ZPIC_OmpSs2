...
#pragma omp task inout(current->J_buf[0; current->overlap_zone]) \
...
#pragma omp task inout(current->J_buf[0; current->total_size]) label(Current Reduction X)
...
#pragma omp task inout(current->J_buf[0; current->total_size]) \
label(Current Smooth X)
void current_smooth_x(t_current *current);

#pragma omp task inout(current->J_buf[0; current->total_size]) label(Current Smooth Y)
