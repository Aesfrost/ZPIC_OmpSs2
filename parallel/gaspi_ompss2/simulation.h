...
typedef struct {
	...
	gaspi_rank_t proc_rank_cart[2];
	int num_procs_cart[2];
	gaspi_rank_t adj_ranks_part[NUM_ADJ_PART]; // Full
	gaspi_rank_t adj_ranks_grid[NUM_ADJ_GRID]; // No diagonals
	...
	t_vfld *gaspi_segm_B;
	t_vfld *gaspi_segm_J;
	t_part **gaspi_segm_part;

	int gaspi_segm_emf_offset[2 * NUM_ADJ_GRID];
	int gaspi_segm_current_offset[2 * NUM_ADJ_GRID];
	int **gaspi_segm_part_offset;
