                                  ...
                                  const bool last_region)
{
	...
	for (int dir = 0; dir < NUM_ADJ_PART; ++dir)
	{
		if(inter_process_comm[dir])
		{
			...
			MPI_Isend(&spec->gaspi_segm_offset_recv[dir], 1, MPI_INT, adj_ranks[dir], notif_id,
			          MPI_COMM_WORLD, &spec->requests[2 * dir]);

			if (dir == PART_RIGHT || dir == PART_LEFT)
				notif_id = NOTIFICATION_ID(dir, region_id, spec_id);
			else notif_id = NOTIFICATION_ID(dir, 0, spec_id);

			MPI_Irecv(&spec->gaspi_remote_offset_send[dir], 1, MPI_INT, adj_ranks[dir], notif_id,
			         MPI_COMM_WORLD, &spec->requests[2 * dir + 1]);
		}else
		{
			...
			spec->requests[2 * dir] = MPI_REQUEST_NULL;
			spec->requests[2 * dir + 1] = MPI_REQUEST_NULL;
		}
	}
}
...
void spec_link_adj_regions(t_species *spec, t_part_vector *adj_spec[8], t_part *part_gaspi_segm,
                           const int *gaspi_segm_offset, const int region_nx[2],
                           const int region_limits[2][2], const int proc_limits[2][2])
{
							 // Left			Centre			Right
	const int size_per_dir[] = {1, 				region_nx[0], 	1,				// Down
	...
	MPI_Waitall(2 * NUM_ADJ_PART, spec->requests, MPI_STATUSES_IGNORE);
                         ...
                         gaspi_rank_t adj_ranks[NUM_ADJ_PART])
{
	...
	for (int dir = 0; dir < NUM_ADJ_PART; dir++)
	{
		...
		if (spec->gaspi_segm_offset_send[dir] >= 0)
		{
			...
			{
				CHECK_GASPI_ERROR(tagaspi_write_notify(PART_SEGMENT_ID(spec_id), 		// Local segment ID
			...
			{
				CHECK_GASPI_ERROR(tagaspi_notify(PART_SEGMENT_ID(spec_id),
				                               adj_ranks[dir],
				                               id,
				                               COMM_PART_WRITE,
				                               queue));
                            ...
                            const gaspi_rank_t adj_ranks[8])
{
	...
	{
		if (spec->gaspi_segm_offset_recv[i] >= 0)
			spec->incoming_part[i].size -= COMM_PART_WRITE;
	...
	for (int i = 0; i < NUM_ADJ_PART; i++)
	{
		...
		if(spec->gaspi_segm_offset_recv[i] >= 0)
		{
			...
			CHECK_GASPI_ERROR(tagaspi_notify(PART_SEGMENT_ID(spec_id), adj_ranks[i],
			                                 notif_id, COMM_PART_ACK, queue));
		}
	}
