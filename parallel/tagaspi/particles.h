...
#define COMM_NPC_FACTOR 4 // Multiplication factor for communication buffers
...
typedef struct {
	...
	MPI_Request requests[2 * NUM_ADJ_PART];
	...
	int gaspi_segm_offset_send[NUM_ADJ_PART];
	int gaspi_segm_offset_recv[NUM_ADJ_PART];
	int gaspi_remote_offset_send[NUM_ADJ_PART];
...
#pragma oss task label("Spec Send") \
	inout(spec->main_vector) \
	inout(spec->incoming_part[PART_DOWN_LEFT]) \
	inout(spec->incoming_part[PART_DOWN_RIGHT]) \
	inout(spec->incoming_part[PART_UP_LEFT]) \
	inout(spec->incoming_part[PART_UP_RIGHT]) \
	onready(spec_receive_ack(spec, region_id, spec_id))
void spec_send_particles(t_species *spec, const int region_id, const int spec_id,
                         gaspi_rank_t adj_ranks[8]);
