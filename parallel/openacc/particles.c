				...
				for (i = range[0][0]; i <= range[0][1]; i++)
				{
					for (k = 0; k < npc; k++)
					{
						if (i + poscell[2 * k] > start)
						{
							spec->part.ix[ip] = i;
							spec->part.iy[ip] = j;
							spec->part.x[ip] = poscell[2 * k];
							spec->part.y[ip] = poscell[2 * k + 1];
							spec->part.safe_to_delete[ip] = false;
							ip++;
						}
					}
				}
...
void spec_inject_particles(t_species *spec, const int range[][2])
{
	...
	int np_inj = (range[0][1] - range[0][0] + 1) * (range[1][1] - range[1][0] + 1) * spec->ppc[0] * spec->ppc[1];
	...
	if (spec->np + np_inj > spec->np_max)
	{
		...
		void *ptr = malloc(spec->np_max * sizeof(*spec->part.ix));
		memcpy(ptr, spec->part.ix, spec->np * sizeof(*spec->part.ix));
		free(spec->part.ix);
		spec->part.ix = ptr;

		ptr = malloc(spec->np_max * sizeof(*spec->part.iy));
		memcpy(ptr, spec->part.iy, spec->np * sizeof(*spec->part.iy));
		free(spec->part.iy);
		spec->part.iy = ptr;

		ptr = malloc(spec->np_max * sizeof(*spec->part.x));
		memcpy(ptr, spec->part.x, spec->np * sizeof(*spec->part.x));
		free(spec->part.x);
		spec->part.x = ptr;

		ptr = malloc(spec->np_max * sizeof(*spec->part.y));
		memcpy(ptr, spec->part.y, spec->np * sizeof(*spec->part.y));
		free(spec->part.y);
		spec->part.y = ptr;

		ptr = malloc(spec->np_max * sizeof(*spec->part.ux));
		memcpy(ptr, spec->part.ux, spec->np * sizeof(*spec->part.ux));
		free(spec->part.ux);
		spec->part.ux = ptr;

		ptr = malloc(spec->np_max * sizeof(*spec->part.uy));
		memcpy(ptr, spec->part.uy, spec->np * sizeof(*spec->part.uy));
		free(spec->part.uy);
		spec->part.uy = ptr;

		ptr = malloc(spec->np_max * sizeof(*spec->part.uz));
		memcpy(ptr, spec->part.uz, spec->np * sizeof(*spec->part.uz));
		free(spec->part.uz);
		spec->part.uz = ptr;

		ptr = malloc(spec->np_max * sizeof(*spec->part.safe_to_delete));
		memcpy(ptr, spec->part.safe_to_delete, spec->np * sizeof(*spec->part.safe_to_delete));
		free(spec->part.safe_to_delete);
		spec->part.safe_to_delete = ptr;
	}
		...
		const t_part_data *uth, const int nx[], t_part_data box[], const float dt, t_density *density)
{
	...
	spec->np_max = npc * nx[1] * nx[0];
	spec->part.ix = malloc(spec->np_max * sizeof(*spec->part.ix));
	spec->part.iy = malloc(spec->np_max * sizeof(*spec->part.iy));
	spec->part.x = malloc(spec->np_max * sizeof(*spec->part.x));
	spec->part.y = malloc(spec->np_max * sizeof(*spec->part.y));
	spec->part.ux = malloc(spec->np_max * sizeof(*spec->part.ux));
	spec->part.uy = malloc(spec->np_max * sizeof(*spec->part.uy));
	spec->part.uz = malloc(spec->np_max * sizeof(*spec->part.uz));
	spec->part.safe_to_delete = malloc(spec->np_max * sizeof(*spec->part.safe_to_delete));


	// Sort
	spec->bin_size = 8;
	spec->n_bins_x = ceil((float) spec->nx[0] / spec->bin_size);
	spec->n_bins_y = ceil((float) spec->nx[1] / spec->bin_size);
	...
	const int range[][2] = {{0, nx[0] - 1}, {0, nx[1] - 1}};
...
//void dep_current_esk(int ix0, int iy0, int di, int dj, t_part_data x0, t_part_data y0, t_part_data x1, t_part_data y1,
//		t_part_data qvx, t_part_data qvy, t_part_data qvz, t_current *current)
//{
//
//	int i, j;
...
//			Wz[i + 4 * j] = S0x[i] * S0y[j] + DSx[i] * S0y[j] / 2.0f + S0x[i] * DSy[j] / 2.0f + DSx[i] * DSy[j] / 3.0f;
//		}
//	}
//
//	// jx
...
//}
//
//void dep_current_zamb(int ix, int iy, int di, int dj, float x0, float y0, float dx, float dy, float qnx, float qny,
//		float qvz, t_current *current)
//{
...
////		dep_current_esk(spec->part.ix[i], spec->part.iy[i], di, dj, spec->part.x[i], spec->part.y[i], x1, y1, qnx, qny,
////				qvz, current);
//
//		dep_current_zamb(spec->part.ix[i], spec->part.iy[i], di, dj, spec->part.x[i], spec->part.y[i], dx, dy, qnx, qny,
//				qvz, current);
//
//		// Store results
//		spec->part.x[i] = x1;
...
//		}
//
//		spec->part.iy[i] += ((spec->part.iy[i] < 0) ? nx1 : 0) - ((spec->part.iy[i] >= nx1) ? nx1 : 0);
//	}
//
...
void spec_deposit_charge(const t_species *spec, t_part_data *charge)
{
	...
	for (i = 0; i < spec->np; i++)
	{
		if(spec->part.safe_to_delete[i]) continue;

		int idx = spec->part.ix[i] + nrow * spec->part.iy[i];
		t_fld w1, w2;
...
//	axis[0] = (t_zdf_grid_axis) {.min = 0.0, .max = spec->box[0], .label = "x_1", .units = "c/\\omega_p"};
//
//	axis[1] = (t_zdf_grid_axis) {.min = 0.0, .max = spec->box[1], .label = "x_2", .units = "c/\\omega_p"};
//
