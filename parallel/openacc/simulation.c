...
static double init_part_energy = 0;
...
void sim_iter(t_simulation *sim)
{
	...
	for (int i = 0; i < sim->n_species; i++)
		spec_advance_openacc(&sim->species[i], &sim->emf, &sim->current);

	// Update current boundary conditions and advance iteration
	current_update_openacc(&sim->current);

	// Advance EM fields
	emf_advance_openacc(&sim->emf, &sim->current);
}
...
//		{
//			usq = part->part[i].ux * part->part[i].ux + part->part[i].uy * part->part[i].uy + part->part[i].uz * part->part[i].uz;
//			gamma = sqrtf(1 + usq);
//			init_part_energy += usq / (gamma + 1);
//		}
//	}
...
void sim_report_energy(t_simulation *sim)
{
	...
	double tot_part = init_part_energy;
	init_part_energy = 0;
