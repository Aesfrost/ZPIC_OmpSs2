...
void spec_prefetch_openacc(t_particle_vector *part, const int device, const int queue)
{
...
void add_block_sum(int *restrict vector, int *restrict block_sum, const int num_blocks,
        const int block_size, const int vector_size, const int queue)
{
	#pragma acc parallel loop gang async(queue)
...
void prefix_sum_min(int *restrict vector, int *restrict block_sum, const int num_blocks,
        const int size, const int queue)
{
	// Prefix sum using a binomial tree
	#pragma acc parallel loop gang vector_length(MIN_WARP_SIZE) async(queue)
...
{
	// Prefix sum using a binomial tree
	#pragma acc parallel loop gang vector_length(LOCAL_BUFFER_SIZE / 2) async(queue)
	for (int block_id = 0; block_id < num_blocks; block_id++)
	{
...
void prefix_sum_openacc(int *restrict vector, const int size, const int queue)
{
	int num_blocks;
	...
	if(size < LOCAL_BUFFER_SIZE / 4)
	{
		...
		prefix_sum_min(vector, block_sum, num_blocks, size, queue);

		if (num_blocks > 1)
		{
			prefix_sum_openacc(block_sum, num_blocks, queue);

			// Add the values from the block sum
			add_block_sum(vector, block_sum, num_blocks, MIN_WARP_SIZE, size, queue);
		}
	} else
	{
		...
		prefix_sum_full(vector, block_sum, num_blocks, size, queue);
		...
		if (num_blocks > 1)
		{
			...
			// Add the values from the block sum
			add_block_sum(vector, block_sum, num_blocks, LOCAL_BUFFER_SIZE, size, queue);
		}
	}
...
void spec_move_vector_int(int *restrict vector, int *restrict temp, int *restrict source_idx,
        int *restrict target_idx, const int move_size, const int queue)
{
	#pragma acc parallel loop async(queue) deviceptr(temp, source_idx)
	for (int i = 0; i < move_size; i++)
		if (source_idx[i] >= 0) temp[i] = vector[source_idx[i]];

	#pragma acc parallel loop async(queue) deviceptr(temp, source_idx, target_idx)
	for (int i = 0; i < move_size; i++)
...
#ifdef ENABLE_PREFETCH
	spec_prefetch_openacc(&spec->main_vector, device, 0);
		...
		t_current *restrict const current, const int limits_y[2], const int device)
{
	...
	int queue = get_gpu_async_queue(device);
	acc_set_device_num(device, DEVICE_TYPE);

	// Advance particles
	#pragma acc parallel loop gang collapse(2) vector_length(THREAD_BLOCK) async(queue)
...
#ifdef ENABLE_PREFETCH
//	spec_prefetch_openacc(&spec->main_vector, device, queue);
	spec_prefetch_openacc(spec->outgoing_part[1], device, queue);
	spec_prefetch_openacc(spec->outgoing_part[0], device, queue);
#endif

	// Check if particles are exiting the left boundary (periodic boundary)
	#pragma acc parallel loop gang vector_length(128) async(queue)
	for(int tile_y = 0; tile_y < spec->n_tiles_y; tile_y++)
	{
        ...
        const int offset_region, const int queue)
{
	...
	#pragma acc parallel loop async(queue) deviceptr(np_per_tile)
	for (int i = 0; i < n_tiles; i++)
		np_per_tile[i] = 0;

	// Calculate the histogram (number of particles per tile) for the main vector
	#pragma acc parallel loop gang collapse(2) async(queue) deviceptr(np_per_tile)
...
void histogram_moving_particles(t_particle_vector *part_vector, int *restrict tile_offset,
		...
		const int old_size, const int queue)
{
	#pragma acc parallel loop gang async(queue)
		...
		const int offset_region, const int sorting_size, const int queue)
{
	...
	#pragma acc parallel loop async(queue) deviceptr(source_idx)
	for(int i = 0; i < sorting_size; i++)
		source_idx[i] = -1;

	#pragma acc parallel loop async(queue) deviceptr(counter)
	for (int i = 0; i < n_tiles; i++)
		counter[i] = mv_part_offset[i];

	// Determine which particles are in the wrong tile
	#pragma acc parallel loop gang collapse(2) async(queue) deviceptr(target_idx, counter)
	for(int tile_y = 0; tile_y < n_tiles_y; tile_y++)
	{
		for(int tile_x = 0; tile_x < n_tiles_x; tile_x++)
		{
	...
	#pragma acc parallel loop gang collapse(2) async(queue) deviceptr(source_idx, target_idx, counter)
	for(int tile_y = 0; tile_y < n_tiles_y; tile_y++)
	{
		for(int tile_x = 0; tile_x < n_tiles_x; tile_x++)
		{
	...
	if(size < old_size)
	{
		#pragma acc parallel loop async(queue) deviceptr(source_idx, counter)
		...
		const int offset_region, const int sorting_size, const int queue)
{
	for(int n = 0; n < 3; n++)
	{
		if(incoming_part[n].enable_vector)
		{
			...
			#pragma acc parallel loop async(queue) deviceptr(target_idx, counter)
			for(int k = 0; k < size_temp; k++)
			{
				int idx;
				int ix = incoming_part[n].ix[k] / TILE_SIZE;
...
void spec_sort_openacc(t_species *spec, const int limits_y[2], const int device)
{
	...
	int max_leaving_np = MAX_LEAVING_PART * spec->main_vector.size_max;
	...
	for(int i = 0; i < 3; i++)
		if(spec->incoming_part[i].enable_vector)
			spec_prefetch_openacc(&spec->incoming_part[i], device, 0);
#endif

	// Calculate the new offset (in the particle vector) for the tiles
	histogram_np_per_tile(&spec->main_vector, spec->tile_offset, spec->incoming_part,
	        spec->n_tiles_y, spec->n_tiles_x, limits_y[0], queue);
	prefix_sum_openacc(spec->tile_offset, n_tiles + 1, queue);
	...
	histogram_moving_particles(&spec->main_vector, spec->tile_offset, spec->mv_part_offset, n_tiles,
	        spec->n_tiles_x, limits_y[0], old_size, queue);
	prefix_sum_openacc(spec->mv_part_offset, n_tiles + 1, queue);

	#pragma acc wait(queue)

	const int sorting_size = spec->mv_part_offset[n_tiles];
	...
	calculate_sorted_idx(&spec->main_vector, spec->tile_offset, source_idx, target_idx, counter,
			spec->mv_part_offset, spec->n_tiles_y, spec->n_tiles_x, old_size, limits_y[0], sorting_size, queue);

	// Sort the main vector
	spec_move_vector_int(spec->main_vector.ix, temp_int, source_idx, target_idx, sorting_size, queue);
	spec_move_vector_int(spec->main_vector.iy, temp_int, source_idx, target_idx, sorting_size, queue);
	spec_move_vector_float(spec->main_vector.x, temp_float, source_idx, target_idx, sorting_size, queue);
	spec_move_vector_float(spec->main_vector.y, temp_float, source_idx, target_idx, sorting_size, queue);
	spec_move_vector_float(spec->main_vector.ux, temp_float, source_idx, target_idx, sorting_size, queue);
	spec_move_vector_float(spec->main_vector.uy, temp_float, source_idx, target_idx, sorting_size, queue);
	spec_move_vector_float(spec->main_vector.uz, temp_float, source_idx, target_idx, sorting_size, queue);

	#pragma acc parallel loop async(queue) deviceptr(source_idx, target_idx)
	for (int i = 0; i < sorting_size; i++)
		if (source_idx[i] >= 0) spec->main_vector.invalid[target_idx[i]] = false;

	// Merge all the temporary vectors into the main vector
	merge_particles_buffers(&spec->main_vector, spec->incoming_part, counter, target_idx,
			spec->n_tiles_x, n_tiles, limits_y[0], sorting_size, queue);
